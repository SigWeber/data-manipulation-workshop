...
```{r include = FALSE}
flights <- flights %>% 
  mutate(short_flights = hour <= 0)

flights %>% 
  filter(short_flights==TRUE)
```
...
delays <- flights %>%
  filter(is.na(dep_delay)==FALSE) %>%  # OR: filter(is.na(dep_delay)==F)
  group_by(dest) %>%
